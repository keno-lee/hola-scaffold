...
const getConfig = (option, isProd) => {
  ...
  let baseConfig = {
    ...
    plugins: [
      ...
      //   format: `build [:bar] ${chalk.green.bold(':percent')} (:elapsed seconds) ${moduleName}`,
      //   clear: false,
      // }),
 ...
 */
const getConfigArray = (modules, isProd) => {
  let configArray = modules.map((moduleItem) => getConfig(moduleItem, isProd));
  // 只在第一个配置中加入插件，是为了防止多个进程多个输出
  configArray[0].plugins = configArray[0].plugins.concat([
    // new ProgressBarPlugin({
    //   format: `\n ${chalk.green.bold('编译中...')} [:bar] ${chalk.green.bold(
    //     ':percent'
    //   )} (:elapsed seconds) :msg \n`,
    //   clear: false,
...
let con = getConfigArray(
  [
    ...
    {
      moduleName: 'japan',
      moduleEntry: './app/japan/main.js',
      cdn: '/',
    },
    {
      moduleName: 'usa/boston',
      moduleEntry: './app/usa/boston/main.js',
      cdn: '/',
    },
    {
      moduleName: 'usa/newyork',
      moduleEntry: './app/usa/newyork/main.js',
      cdn: '/',
    },
  ],
  true
