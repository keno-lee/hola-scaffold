...
const publishProject = (args) => {
    ...
    if (!devServerUrl) {
        console.log();
        console.log(chalk_1.default.red('缺少参数: 远程服务器地址!'));
        console.log();
        return;
    }
    let sourceFilePathList = []; // 目标文件路径数组
    let noFoundModule = []; // 收集未找到的模块
    if (noconfigs) {
        sourceFilePathList = getFolderFilePath(path_1.default.join(process.cwd(), sourceFolderName));
    }
    else {
        // 如果所选模块为空，则拿到所有模块
        if (modules.length <= 0) {
        ...
        modules.forEach((moduleName) => {
            let sourceFolderPath = path_1.default.join(process.cwd(), sourceFolderName, moduleName);
            // 判断是否存在需要上传文件夹的目录
            if (!fs_1.default.existsSync(sourceFolderPath)) {
 ...
 */
const uploadHttpService = (serverUrl, sourcePath, destPath) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
        request_1.default(`${serverUrl}/upload`, {
            timeout: 20000,
            method: 'PUT',
            headers: {
            ...
            formData: {
                dest: destPath,
                file: fs_1.default.createReadStream(sourcePath),
            },
        }, (error) => {
            if (error) {
 ...
 */
const getFolderFilePath = (folderPath) => {
    let pathArray = [];
    const files = fs_1.default.readdirSync(folderPath); // 文件夹目录
    files.forEach((file) => {
...
const create2run = (serverUrl, sourceFilePathList, noFoundModule) => {
    ...
    sourceFilePathList.forEach((filePath) => {
        let fileRelativePath = path_1.default.relative(process.cwd(), filePath);
        ...
        uploadQuene.push((callback) => {
            ...
            uploadHttpService(serverUrl, filePath, fileRelativePath)
                .then(() => {
                console.log(`${chalk_1.default.cyan(fileRelativePath)} => ${chalk_1.default.green('上传成功')}`);
                callback(null);
            })
                .catch((err) => {
                callback(err);
    ...
    // 执行任务队列
    async_1.default.parallel(uploadQuene, (err) => {
        if (err) {
            console.log();
            console.log(chalk_1.default.red(`upload failed => ${err}`));
            console.log();
        }
        else {
            console.log();
            console.log(chalk_1.default.green(`可用模块上传成功`));
            console.log();
            if (noFoundModule.length > 0) {
                console.log(chalk_1.default.red(`未找到模块: ${noFoundModule}`));
                console.log();
            }
        }
    });
};
exports.default = publishProject;
